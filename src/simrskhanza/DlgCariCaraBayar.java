...
//            iyem=null;
    ...
    private void tampil2() {
        Valid.tabelKosong(tabMode);
        try {
            ps=koneksi.prepareStatement("select * from penjab where (penjab.kd_pj like '%"+ 
                    TCari.getText() + "%' or penjab.png_jawab like '%"+ TCari.getText() +"%') " +
                    "and penjab.status='1' order by penjab.png_jawab");
            try{           
                rs=ps.executeQuery();
                i=1;
                while(rs.next()){
