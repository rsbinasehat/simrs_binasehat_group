...
public final class DlgCariPoli extends javax.swing.JDialog {
    ...
    */
    ...
    private void tampil2() {
        Valid.tabelKosong(tabMode);
        try {
            ps = koneksi.prepareStatement("select * from poliklinik where " +
                    "(poliklinik.kd_poli like '%"+ TCari.getText() +"%' or " +
                    "poliklinik.nm_poli like '%"+ TCari.getText() +"%') and poliklinik.status='1'");
            try {
                rs = ps.executeQuery();
                while (rs.next()) {
                    tabMode.addRow(new Object[]{rs.getString(1), rs.getString(2), Valid.SetAngka(rs.getDouble(3)), Valid.SetAngka(rs.getDouble(4))});
